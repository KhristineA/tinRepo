/*
 * File: app/view/MovieViewController.js
 *
 * This file was generated by Sencha Architect version 4.2.4.
 * http://www.sencha.com/products/architect/
 *
 * This file requires use of the Ext JS 6.6.x Classic library, under independent license.
 * License of Sencha Architect does not include license for Ext JS 6.6.x Classic. For more
 * details see http://www.sencha.com/license or contact license@sencha.com.
 *
 * This file will be auto-generated each and everytime you save your project.
 *
 * Do NOT hand edit this file.
 */

Ext.define('Vidly.view.MovieViewController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.vmmovie',

    show : function() {
        "use strict";
 
        var me = this;
        me.getView().show();
 
        //var context = win.context ? win.context.initialize() : me.setupContext();
        var context = me.setupContext();
        context.data.load();
    },

    setupContext: function() {
        var me = this,
            win = me.getView();
 
        win.context = Ext.create('iRely.Engine', {
            window : win,
            store  : Ext.create('Vidly.store.Movie'),
            singleGridMgr: Ext.create('iRely.grid.Manager', {
                grid:  win.down('grid'),
                title: 'Movie Grid',
                columns: [
                    {
                        xtype: 'gridcolumn',
                        itemId: 'colName',
                        dataType: 'string',
                        dataIndex: 'strName',
                        text: 'Movie Name',
                        flex: 1,
                        editor: {
                            xtype: 'textfield',
                            itemId: 'txtName'
                        }
                    },
                    {
                        xtype: 'gridcolumn',
                        itemId: 'colGenreId',
                        dataType: 'numeric',
                        dataIndex: 'intGenreId',
                        text: 'Genre Id',
                        flex: 1,
                        editor: {
                            xtype: 'textfield',
                            itemId: 'txtGenreId'
                        }
                    },
                    // {
                    //     xtype: 'gridcombobox',
                    //     columns: [
                    //         {
                    //             dataIndex: 'intGenreId',
                    //             dataType: 'numeric',
                    //             hidden: true
                    //         },
                    //         {
                    //             dataIndex: 'strName',
                    //             dataType: 'string',
                    //             text: 'Genre',
                    //             flex: 1
                    //         }
                    //     ],
                    //     itemId: 'cboGenre',
                    //     margin: '0 5 5 0',
                    //     fieldLabel: 'Genre',
                    //     labelWidth: 130,
                    //     displayField: 'strName',
                    //     valueField: 'strName'
                    // },
                    {
                        xtype: 'datecolumn',
                        itemId: 'colDateAdded',
                        dataType: 'date',
                        dataIndex: 'dtmDateAdded',
                        text: 'Date Added',
                        flex: 1
                    },
                    {
                        xtype: 'datecolumn',
                        itemId: 'colReleaseDate',
                        dataType: 'date',
                        dataIndex: 'dtmReleaseDate',
                        text: 'Release Date',
                        flex: 1,
                        editor: {
                            xtype: 'textfield',
                            itemId: 'txtReleaseDate'
                        }
                    },
                    {
                        xtype: 'gridcolumn',
                        itemId: 'colNumberInStock',
                        dataType: 'numeric',
                        dataIndex: 'intNumberInStock',
                        text: 'Number In Stock',
                        flex: 1,
                        editor: {
                            xtype: 'textfield',
                            itemId: 'txtNumberInStock'
                        }
                    }
                ]
            })
        });
 
        return win.context;
    }
});
